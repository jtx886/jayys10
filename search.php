...
$items = array(); $totalPages = 0; $total = 0;
if ($q !== '') {
    ...
    $total = isset($data['total_results']) ? intval($data['total_results']) : 0;
    /* 过滤人物结果 */
    $items = array_values(array_filter($items, function ($r) { return isset($r['media_type']) && $r['media_type'] !== 'person' && !empty($r['poster_path']); }));
}
...
<div class="page-head anim">
  <span class="s-ico" style="width:42px;height:42px;border-radius:12px;background:var(--theme-soft);color:var(--theme);display:flex;align-items:center;justify-content:center"><i class="ico i-search" style="font-size:19px"></i></span>
  <h1><?php echo $q !== '' ? '搜索：' . h($q) : '搜索'; ?></h1>
  <?php if ($q !== ''): ?><span class="count">找到 <?php echo $total; ?> 条结果</span><?php endif; ?>
</div>

<?php if ($q === ''): ?>
<div class="empty-box"><i class="ico i-search"></i><p>输入影视名称开始搜索吧</p></div>
<?php elseif (empty($items)): ?>
<div class="empty-box"><i class="ico i-info"></i><p>未找到与「<?php echo h($q); ?>」相关的影视</p><a class="btn ghost" href="index.php">返回首页</a></div>
<?php else: ?>
